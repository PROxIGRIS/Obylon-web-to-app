...
class WorkstationGuard:
    """
    Enforcement layer: hardware input locking and process termination.

    Input lock temporarily suppresses all keyboard and mouse events via
    pynput suppress=True listeners.  Releases on admin remote command,
    explicit timeout, or hardware panic hotkey (Ctrl+Alt+Shift+P).
    """
    ...
    def terminate_process(self, target_name: str) -> bool:
        """Terminate a named process. Called by admin 'kill_task' command."""
        if not target_name:
            return False
    ...
    def _input_lock_loop(self) -> None:
        """Suppress all input until system_frozen is cleared."""
        def _noop(*args, **kwargs):
            pass

        self._k_listener = keyboard.Listener(
            on_press=_noop, on_release=_noop, suppress=True
        )
        self._m_listener = mouse.Listener(
            on_click=_noop, on_scroll=_noop, on_move=_noop, suppress=True
        )
    ...
    def lock_workstation(self, duration: int = None) -> None:
        """Engage hardware input suppression."""
        with self._lock:
    ...
    def disengage_freeze(self) -> None:
        """Release hardware input suppression."""
        with self._lock:
...
class LevEngine:
    ...
    def calculate_typo_modifier(inp: str, tgt: str) -> float:
        ...
        if avg <= 1.5 and len(tgt) > 4:
            return 0.85   # Adjacent slip on a long word → probably a typo
        return 1.10        # Cross-keyboard on short word → probably deliberate
...
def _axiom_layer2_arbitrate(live_signals: list[dict]) -> float:
    ...
    2. Compute a base confirmed score from the highest Lev result and
       the mean weighted signal strength.
    3. Apply co-occurrence multipliers for dangerous kind combinations.
    4. Return confirmed_score in [0, 1].

    This runs in a background thread spawned by axiom_evaluate().
    """
    if not live_signals:
        return 0.0
    ...
    for sig in live_signals:
        if not sig["value"]:
            continue
        c_lev, _, _ = LEV.evaluate_suspicion(sig["value"], "")
    ...
    # Base confirmed score blends Lev result and signal intensity
    base = (highest_lev * 0.55) + (min(mean_w * 2.0, 1.0) * 0.45)

    # ── Step 3: Co-occurrence multipliers ────────────────────────────────────
...
def usb_and_adapter_monitor(workstation_id: str) -> None:
    ...
    try:
        ...
        def _watch_disks():
            while True:
                try:
                    ...
                    if "wmi" not in str(e).lower():
                        print(f"[usb] disk watcher: {e}", file=sys.stderr)
        ...
        threading.Thread(target=_watch_disks,    daemon=True, name="usb_disk").start()
...
def classify_web_context(dom_text: str) -> tuple[bool, str]:
    ...
    if not dom_text or len(dom_text) < 20:
        return False, ""
    score, hits, tl = 0, [], dom_text.lower()
    for cat, words in WEB_WEIGHTS.items():
        w = 20 if cat == "hardcore" else 15 if cat == "explicit" else 5
        for word in words:
...
def fire_alert(
    ...
    if severity == "critical" and not is_whitelisted:
        ...
        print("[guard] Critical violation. Workstation locked for admin review.")
    elif severity == "critical" and is_whitelisted:
        print(
            f"[strike] Critical on whitelisted '{proc}' — "
...
def extract_ocr_suspicion(image_bytes: Optional[bytes]) -> float:
    ...
    try:
        ...
        if not raw_text.strip():
            return 0.0
        score, _, _ = LEV.evaluate_suspicion(raw_text.strip().lower(), "")
        return score
    except Exception as e:
...
def _calculate_final_arbitration(
    ...
    Weights shift dynamically when a signal is absent (0.0).
    """
                ...
                if cmd == "terminate":
                    ...
                    if download_url:
                        try:
                            ...
                            Path("phoenix.bat").write_text(bat)
...
def main() -> None:
    ...
    threads = [
        threading.Thread(target=heartbeat_loop,               args=(wid,),  daemon=True, name="heartbeat"),
        threading.Thread(target=scan_loop,                     args=(wid,),  daemon=True, name="scan_loop"),
        threading.Thread(target=action_loop,                   args=(wid,),  daemon=True, name="action_loop"),
        ...
        threading.Thread(target=sync_daemon,                                  daemon=True, name="sync_daemon"),
        threading.Thread(target=boot_optics_server,                           daemon=True, name="optics_server"),
        threading.Thread(target=_background_keylogger,                        daemon=True, name="keylogger"),
        # v7.0 new threads
        threading.Thread(target=_background_clipboard_monitor,                daemon=True, name="clipboard_monitor"),
        threading.Thread(target=wmi_process_monitor,           args=(wid,),  daemon=True, name="wmi_monitor"),
        threading.Thread(target=usb_and_adapter_monitor,       args=(wid,),  daemon=True, name="usb_monitor"),
    ]

    for t in threads:
        t.start()

    print(f"[system] {len(threads)} daemon threads armed.")
