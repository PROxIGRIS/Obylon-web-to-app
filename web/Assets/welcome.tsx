...
import {
  ...
  Sparkles, ArrowRight, Wand2, Shield,
} from "lucide-react";
...
const ProgressBar = memo(({ step }: { step: number }) => (
  <div className="absolute top-8 left-1/2 -translate-x-1/2 z-20" role="navigation" aria-label="Setup progress">
...
function WelcomePage() {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-background overflow-hidden" role="main">
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
        <motion.div className="absolute top-[-20%] left-[-10%] w-[60%] h-[60%] rounded-full blur-[140px] animate-pulse"
          style={{ background: "radial-gradient(ellipse,rgba(140,20,20,0.15) 0%,transparent 70%)", x: parallaxX, y: parallaxY }} />
        <div className="absolute bottom-[-20%] right-[-10%] w-[50%] h-[50%] rounded-full blur-[120px] animate-pulse"
          style={{ animationDelay:"1.3s", background:"radial-gradient(ellipse,rgba(80,10,10,0.12) 0%,transparent 70%)" }} />
        <div className="absolute top-[30%] right-[20%] w-[30%] h-[30%] rounded-full blur-[80px] animate-pulse"
          style={{ animationDelay:"2.4s", background:"radial-gradient(ellipse,rgba(30,30,30,0.22) 0%,transparent 70%)" }} />
        <div className="absolute inset-0" style={{
          backgroundImage: "repeating-linear-gradient(0deg,transparent,transparent 40px,rgba(255,255,255,0.4) 40px,rgba(255,255,255,0.4) 41px)",
          opacity: 0.022,
        }} />
      </div>
          ...
          {step === 0 && (
            <motion.div key="welcome" custom={dir} variants={pageVars} initial="enter" animate="center" exit="exit"
              ...
              <motion.div
                ...
                aria-label="Obylon orb — hold 3 seconds for a surprise"
              >
                <SuminagashiOrb mouseX={mousePos.x} mouseY={mousePos.y}
                  strengthScore={0} step={0} isComplete={isExiting}
                  holdProgress={holdProgress} typingVelocity={0} size={200} />
                <GlitchEasterEgg visible={easterEgg} />
              </motion.div>

              <motion.div initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.4, duration:0.6 }} className="space-y-4">
                <div className="flex items-center justify-center gap-2 text-[10px] uppercase tracking-[0.45em] font-mono" style={{ color:"var(--blood)" }}>
                  <span className="w-1.5 h-1.5 rounded-full bg-blood heartbeat-dot" aria-hidden="true" />
                  Obylon by Umbraxis
                </div>
                <h1 className="font-serif text-[clamp(2.5rem,8vw,4.5rem)] font-bold leading-tight tracking-tight break-all max-w-[90vw]">
                  {(() => {
                    const first = user?.email?.split("@")[0] || "Operator";
                    return (
                      <span className="bg-clip-text text-transparent"
                        style={{ backgroundImage:"linear-gradient(to right,var(--charcoal),var(--blood),var(--charcoal))" }}>
          ...
          {step === 2 && (
            <motion.div key="password" custom={dir} variants={pageVars} initial="enter" animate="center" exit="exit"
              ...
              <motion.div custom={0} variants={fieldVariants} initial="hidden" animate="visible" className="text-center mb-6">
                <div className="flex justify-center mb-5 relative" style={{ width:120, margin:"0 auto" }}>
                  <SuminagashiOrb mouseX={mousePos.x} mouseY={mousePos.y}
                    strengthScore={pwdStrength.score} step={2}
                    isComplete={isExiting} holdProgress={0}
                    typingVelocity={typingVelocity} size={120} />
                  <AnimatePresence>
                    {lockEngaged && (
      ...
      <div className="absolute bottom-6 text-center z-10" aria-hidden="true">
