...
# --- STANDALONE EDR PATH RESOLUTION ---
# Automatically routes the sniper to the embedded engine inside the PyInstaller temp folder
...
for _sev, _patterns in LEXICON.items():
    for _pat in _patterns:
        ...
        for rule in clean.split("|"):
            ...
            if " " in rule or "[" in rule or "\." in rule: continue
...
# =====================================================
# PHASE 2, 3 & 4: OCR SNIPER, ROUTING, & ARBITRATION
# =====================================================
...
def extract_ocr_suspicion(image_bytes: bytes | None) -> float:
    ...
    try:
        ...
        print(f"[ocr] Sniper failure: {e}", file=sys.stderr)
        return 0.0
                ...
                if cmd == "terminate":
                    ...
                    if download_url:
                        try:
                            ...
                            # 2. Syntax-safe, filename-agnostic batch script
                            bat_script = (
                                "@echo off\n"
                            ...
                            # 3. Cross-platform safe subprocess execution
                            if platform.system() == "Windows":
...
def main() -> None:
    ...
    print("═" * 60)
    print(" [+] Architecture : Master Sovereign Build (LTS)")
    print(" [+] Intelligence : Lev Engine Active & Armed")
    print(" [+] Enforcement  : Dual-Gated Hardware Warden")
